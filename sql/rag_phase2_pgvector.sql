...
create index if not exists idx_rag_embedding_kb_id
    on rag_embedding (knowledge_base_id);

create index if not exists idx_rag_embedding_document_id
    on rag_embedding (document_id);

create index if not exists idx_rag_embedding_chunk_id
    on rag_embedding (chunk_id);

create index if not exists idx_rag_embedding_model
    on rag_embedding (embedding_model);

create index if not exists idx_rag_embedding_create_time
    on rag_embedding (create_time);

create index if not exists idx_rag_embedding_vector_cosine
